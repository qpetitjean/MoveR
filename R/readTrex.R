...
readTrex = function(trexPath,
  ...
  indiv_list <-
    ...
    tidyverse::set_names(paste("data", Trex_ind_names, sep = "_")) 
  ...
  rep(indiv_list[[1]]$files, times = length(indiv_list))) %>%
    tidyverse::set_names(paste(Trex_ind_names, rep(indiv_list[[1]]$files, times = length(indiv_list)), sep =
                      "_"))
  ...
  df_list_missing <-
    map(df_list_missing_temp3, ~ rowid_to_column(.x, "frame_numb"))
  missing_track <- as.data.frame(df_list_missing %>%
                                   map_df(bind_rows, .id = "identity"))

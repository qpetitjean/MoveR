...
#'
#' @param viDur A numeric value expressed in frames, the length/duration of the video (mandatory)
#'
...
#'  i = data.frame(rPath[1], rPath[2], rPath[3])
#'  names(i) = c("x.pos", "y.pos", "frame")
#'  fragsList = list.append(fragsList, i)
#'}
...
drawFrags <- function(fragsList,
  ...
  if (is.na(viDur) == TRUE) {
    stop(
      "viDur argument should be define, use track_stats to determine the number of frame within the video"
    )
  } else {
    ...
    # if an additional function is added to draw_frags draw it
    if (!is.null(add2It) == TRUE) {
      add2It
    }
    
    # if timeWin is defined, color only the fraction of fragments within time window interval(s)
    dfColoration <- cbind(replicate(length(timeWin), coloration))
    
    for (i in 1:length(timeWin)) {
      if (timeWin[[i]][1] > 0) {
        lower <- timeWin[[i]][1] - 1
    ...
    if ((NA  %in% selFrags) == TRUE) {
      for (f in 1:length(fragsList)) {

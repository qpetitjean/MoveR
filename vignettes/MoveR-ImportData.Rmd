...
Using the <em>drawFrags</em>` function, it is easy to visualize the trajectories included in the dataset (see fig. \@ref(fig:Drawfrags1)A below), or to focus on the trajectories described only by some particles, here the fifth' first (see fig. \@ref(fig:Drawfrags1)B below).
...
By looking at the fig. \@ref(fig:Drawfrags3)B, we can see that some particles' tracks are interrupted. This is particularly obvious for Tricho_7 or Tricho_11, let's take a closer look. 

```{r Drawfrags4, echo=TRUE, fig.align='center', message=FALSE, warning=FALSE, out.width="50%", fig.width=7, fig.height=5, dpi = 300, fig.cap="Micro-wasps trajectories for Tricho_7 and Tricho_11 over the whole video timeline."}
...
<strong>Meanwhile enjoy coding.</strong>
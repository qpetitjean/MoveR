...
Briefly, this dataset come from the video recording (image resolution: 3840x2160) of 12 parasitic micro-wasps (genus <em>Trichogramma</em>) placed in a polygonal arena for 8 minutes at 25 fps and tracked using <a href="https://trex.run">TRex</a> (see fig. \@ref(fig:VideoPics) below). 

```{r VideoPics, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width="100%", fig.cap="Frame extracted from a video recording of 12 parasitic micro-wasps (genus <em>Trichogramma</em>) placed in a polygonal arena."}
knitr::include_graphics(
  "https://github.com/qpetitjean/MoveR_SampleData/raw/main/sample_1/2602_ISA3080_Low_5.mov_3600.png"
)
...
Using the <em>drawFrags</em>` function, it is easy to visualize the trajectories included in the dataset (see fig. \@ref(fig:Drawfrags1)A below), or to focus on the trajectories described only by some particles, here the tens' first (see fig. \@ref(fig:Drawfrags1)B below).

Note that <em>drawFrags</em> needs an argument specifying the timeline of the video (here "frame"). One can also specify the resolution of the video to set x and y limits of the plot but as default, the imgRes argument use the maximum values recorded in x and y + 5% to set the plots limits (see drawFrags documentation).

```{r Drawfrags1, echo=TRUE, fig.show="hold", fig.align='center', message=FALSE, warning=FALSE, out.width="100%", fig.width=12, fig.height=5, dpi = 300, fig.cap="Micro-wasps trajectories over the whole video timeline, expressed in frame. A. all trajectories are represented. B. only the first 10 tracklets are represented. The trajectories are colored according to the moments at which they are recorded."}
...
otherwise, it is possible to focus on the trajectories recorded within a given time intervals. For instance, between 1 and 999 frames (see fig. \@ref(fig:Drawfrags2)A below) and between 6000 and 6999 frames and 11000, 11999 frames (see fig. \@ref(fig:Drawfrags2)B below).
...
Note that it is also possible to represent specified tracklets over given time window(s) by combining the selFrags argument and timeWin argument respectively.

Finally, drawFrags allow to color the particles' tracks according to categorical variable, such as the identity of the particle (see fig. \@ref(fig:Drawfrags3)A below). 
However, here the identity of the particles is a numerical variable, we hence need to convert it to a factor, for instance by appending "Tricho" in front of each id. For this purpose, the easiest way is to use the convert2list utility, transform the identity variable then convert back the data as a list of tracklet using convert2frags as follow: 

```{r convertId}
trackDatList <- MoveR::convert2list(trackDat)
trackDatList[["identity"]] <- paste("Tricho", trackDatList[["identity"]], sep = "_")
trackDat <- MoveR::convert2frags(trackDatList, by = "identity")
str(trackDat)
```

As previously noted, the various arguments from the drawFrags function can be combined to emphasize the behavior of particular tracklets over a specified time window (see fig. \@ref(fig:Drawfrags3)B below).

```{r Drawfrags3, echo=TRUE, fig.align='center', message=FALSE, warning=FALSE, out.width="100%", fig.width=12, fig.height=5, dpi = 300, fig.cap="Micro-wasps trajectories A. over the whole video timeline, and B. over a time window of 1000 frames (between 1 and 999 frames). Here the trajectories are colored according to the identity of the particle."}
# split the graphical window in 2 columns
par(mfrow=c(1,2))
...
The drawFrags function also have various arguments helping to customize the graphical representation, see the function documentation for further information.

That's all for this first introduction to the MoveR package. 
For more insight on how to clean/filer the dataset as well as perform further computations check the next tutorials. 

<strong>Enjoy coding.</strong>
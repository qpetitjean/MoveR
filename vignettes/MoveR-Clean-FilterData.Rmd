...
trackDat2 <- MoveR::analyseTracklets(trackDat.Infilt[[2]],
                                     customFunc =
                                       list(indLengthcm = function(x) x[["maj.ax"]] * MoveR::getInfo(TRexDat, "scale")))
...
trackDat.lenfilt <-
  MoveR::filterTracklets(trackDat2,
                     filter.length,
                     splitCond = TRUE,
                     minDur = MoveR::getInfo(TRexDat, "frameR"))
...
ArenaEdge <- MoveR::circles(
  mean(edge[["x.pos"]]),
  mean(edge[["y.pos"]]),
  radius = 2.5 / 2 / MoveR::getInfo(TRexDat, "scale"), # here the radius of the arena is 2.5cm/2 divided by the scaling to obtain the value in pixels
  border = "firebrick",
  draw = T
)
...
Data_Trex_stats_before_filter <-
  MoveR::summary(trackDat,
                    frameR = MoveR::getInfo(TRexDat, "frameR"),
                    scale = MoveR::getInfo(TRexDat, "scale"),
                    unit = "cm")

Data_Trex_stats_after_filter <-
  MoveR::summary(trackDat5,
                    frameR = MoveR::getInfo(TRexDat, "frameR"),
                    scale = MoveR::getInfo(TRexDat, "scale"),
                    unit = "cm")
